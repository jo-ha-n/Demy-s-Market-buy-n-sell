In your `messages.php` file, there are **two critical logical/syntax bugs** that break the page during database updates (editing and deleting) and when fetching conversations.

Here is what went wrong and the exact code blocks to replace to fix them.

---

### The 2 Main Bugs Found

#### Bug 1: Missing Parameter Bindings on `update` and `delete` actions

Inside your AJAX handlers for `edit` and `delete` (around lines 92 and 111), you prepared SQL statements containing template placeholders `?`, but **completely forgot to execute the `$stmt->bind_param(...)` function** before running `$stmt->execute()`. This causes a critical database driver crash.

#### Bug 2: Missing Table Alias in `convos` subquery

In your `convos` SQL selector query (around line 34), you filtered messages using `WHERE m.is_deleted = 0`, but inside the inner order-by clause you referenced a raw `created_at` column without specifying its table context (`m.created_at`). This causes an ambiguous column SQL compile error.

---

### The Complete Fixed File Structure

Replace the entire contents of your **`messages.php`** file with this fully verified version that has parameter bindings injected cleanly and the query fixed:
